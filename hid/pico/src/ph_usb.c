...
#define _CHECK_MOUSE(x_mode)                                 \
	{                                                        \
		if (_mouse_iface < 0 || !PH_O_IS_MOUSE_USB_##x_mode) \
		{                                                    \
			_MOUSE_CLEAR;                                    \
			return;                                          \
		}                                                    \
...
const u8 *tud_descriptor_device_cb(void)
{
	...
	if (ph_g_is_bridge)
	{
		...
		desc.idProduct = 0xC52C; // USB Receiver (bridge)
	}
...
const u16 *tud_descriptor_string_cb(u8 index, u16 lang_id)
{
	...
	else
	{
		...
		case 1:
			...
			break; // Manufacturer
		case 2:
			strcpy(str, (ph_g_is_bridge ? "USB Receiver" : "USB Receiver"));
			break; // Product
		case 3:
			pico_get_unique_board_id_string(str, 32);
			break; // Serial
		case 4:
		{
			if (ph_g_is_bridge)
			{
				strcpy(str, "USB Receiver CDC");
			}
			else
			{
				return NULL;
